...
public partial class TicTacToeServerForm : Form
{
    ...
    internal byte[] board; // the local representation of the game board  
    internal static Player[] players; // two Player objects                      
    ...
    public bool GameOver(char stafur)
    {
        if (
            board[0] == stafur && board[1] == stafur && board[2] == stafur
            ||
            board[0] == stafur && board[4] == stafur && board[8] == stafur
            ||
            board[0] == stafur && board[3] == stafur && board[6] == stafur
            ||
            board[3] == stafur && board[4] == stafur && board[5] == stafur
            ||
            board[6] == stafur && board[7] == stafur && board[8] == stafur
            ||
            board[1] == stafur && board[4] == stafur && board[7] == stafur
            ||
            board[2] == stafur && board[5] == stafur && board[8] == stafur
            ||
            board[2] == stafur && board[4] == stafur && board[6] == stafur)
        {
            return true;
        }
...
public static class Data 
{
    ...
    {
        foreach (var player in TicTacToeServerForm.players)
        {
            player.writer.Write(text);
        }
    }
}
        ...
        while (true)
        {
            ...
            while (!done)
            {
                ...
                if (server.GameOver('X'))
                {
                    ...
                    Data.send("Spilari 1 vann, með X");
                    Data.send("apakottur");
                    if (reader.ReadString() == "apahundur")
                    {
                        MessageBox.Show("þett aer tyopoypy");
                    }
                    break;
                }
                else if (server.GameOver('O'))
                {
                    Data.send("Spilari 2 vann, með O");
                    Data.send("apakottur"); 
                    if (reader.ReadString() == "apahundur")
                    {
                        MessageBox.Show("þett aer tyopoypy");
                ...
                for (int i = 0; i < server.board.Length; i++)
                {
                    if (server.board[i] == 0)
                    {
                        break;
                    }
                    else if ((server.board[i] != 0) && (i == 8))
                    {
                        done = true;
                        Data.send("Jafntefli");
                        Data.send("apakottur"); 
